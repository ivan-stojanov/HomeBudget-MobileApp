...
html5rocks.indexedDB.open = function() {	
	...
	request.onsuccess = function(e) {  
		...
		requestID.onsuccess = function(e) {	
			...
			var listDatesTest = (result.incomeCreated).split("+");
			...
			if(countlistDatesSummary > 1){
				$('.incomeTotalAmount').show();
				$('.incCreatedTXT').hide();
				//listDatesSummary = //var listDates = (result.incomeCreated).split("+");
				$('#incomeCreatedList').html("<hr>List of Incomes (by Date):<br>");
				for(var countDates = 0; countDates < countlistDatesSummary; countDates++) {
					var arrayCreationInstanceDate = (listDatesSummary[countDates]).split("/");
					...
					$('#incomeCreatedList').html($('#incomeCreatedList').html() + "<input type='submit' value='Delete this instance' onclick=deleteIncome('" + result.incomeCreated + "','" + listDatesSummary[countDates] + "','" + countDates + "','" + countlistDatesSummary + "','" + result.incomeBillPaid + "')>");	
				}	
			} else {
				$('.incomeTotalAmount').hide();
				var arrayCreationDate = (listDatesSummary[0]).split("/");
	...
	if(totalPositions == 1) {
		...
		if(confirm("Are you sure you want to delete this instance of the income created at " + dateToDelete + " ?")){
			...
			requestDelete.onsuccess = function(e) {
				...
				window.location.href = "incomeDetailsSummary.html";
				return true;				
			}			
			requestDelete.onerror = function(e) {
				alert('request.onerror!');
