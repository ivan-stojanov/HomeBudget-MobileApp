...
html5rocks.indexedDB.open = function() {	
	...
	request.onsuccess = function(e) {  
		...
		requestID.onsuccess = function(e) {	
			...
			var listDatesTest = (result.expenseCreated).split("+");
			var listStatusPaidTest = (result.expenseBillPaid).split("+");
			var listDatesSummary = new Array();
			var listPaidStatusSummary = new Array();
			var countlistDatesSummary = 0;
			var datePartsIterate;	var dateFormatCreated;	var dateNow;
			for(var countDatesTest = 0; countDatesTest < listDatesTest.length; countDatesTest++) {
				datePartsIterate = listDatesTest[countDatesTest].split("/");	//	17/04/2014/23/59
				dateFormatCreated = new Date(datePartsIterate[2],datePartsIterate[1] - 1,datePartsIterate[0],datePartsIterate[3],datePartsIterate[4]);
				dateNow = new Date();
				if(conditionItem == "today"){
					condition = ((dateFormatCreated - todayStartDate > 0) && (dateNow - dateFormatCreated > 0));
				} else if(conditionItem == "past7days"){
					condition = ((dateFormatCreated - weekStartDate > 0) && (dateNow - dateFormatCreated > 0));
				} else if(conditionItem == "thisMonth"){
					condition = ((dateFormatCreated - monthStartDate > 0) && (dateNow - dateFormatCreated > 0));
				} else if(conditionItem == "thisYear"){
					condition = ((dateFormatCreated - yearStartDate > 0) && (dateNow - dateFormatCreated > 0));
				} else {
					condition = true;
				}
				...
				if(condition){
					if(result.expenseCategory == "Bill"){
						if(listStatusPaidTest[countDatesTest] == "paidYes"){
							listDatesSummary[countlistDatesSummary] = listDatesTest[countDatesTest];
							listPaidStatusSummary[countlistDatesSummary] = "paidYes";
							countlistDatesSummary++;
						}
					} else {
						listDatesSummary[countlistDatesSummary] = listDatesTest[countDatesTest];
						listPaidStatusSummary[countlistDatesSummary] = "paidYes";
			...
			if(result.expenseCategory == "Bill"){
				if(countlistDatesSummary > 1){	//if more then one bill is paid
					$('.expenseTotalAmount').show();
					$('.exCreatedTXT').hide();
					var listDates = listDatesSummary;//(result.expenseCreated).split("+");//
					var listPaidStatus = listPaidStatusSummary;//(result.expenseBillPaid).split("+");//
					var array1UseForChangeStatus = (result.expenseBillPaid).split("+");
					var array2UseForChangeStatus = (result.expenseCreated).split("+");
					var numPaidItems = 0;				
					...
					for(var countDates = 0; countDates < listDates.length; countDates++) {
						...
						if(listPaidStatus[countDates] == "paidYes"){
							var arrayOriginalDateString = result.expenseCreated.split("+");
							var findIndexCounter = 0;
							for(var countInd = 0; countInd < arrayOriginalDateString.length; countInd++) {
								if(arrayOriginalDateString[countInd] == listDates[countDates]){
									findIndexCounter = countInd;
								}
							}
					...
					var listPaidStatusAmount1 = listPaidStatusSummary;//(result.expenseBillPaid).split("+");
					...
					var listPaidStatusDate1 = listDatesSummary;//(result.expenseCreated).split("+");
				...
				if(result.expenseNumItems > 1){
					...
					$('.expenseTotalAmount').hide();
					var arrayCreationDate = listDatesSummary;//(result.expenseCreated).split("/");
					var printCreationDate = arrayCreationDate[0] + "/" + arrayCreationDate[1] + "/" + arrayCreationDate[2] + " at " + arrayCreationDate[3] + ":" + arrayCreationDate[4];
					$('#expenseCreated').text(printCreationDate);
				}				
			}
			...
			if (openedIndexx) {					
				...
				curCursorA.onsuccess = function(evt) {
					...
					if (cursorA) {
						...
						window.location.href = "expenseDetailsSummary.html";
					}
				}
 	...
 	if(totalPositions == 1) {
 		if(confirm("This will delete all of instances of the expense in the system. Are you sure you want to completely delete this expense?")){		
 			...
 			requestDelete.onsuccess = function(e) {
 				...
 				window.location.href = "expensesListSummary.html";
 				return true;				
 			}			
	...
	//AKO E BILL ZAMENI GO STATUSO VO UNPAID, SO KODOT OD billDetails.js - LINE 120
		...
		requestDelete.onsuccess = function(e) {  
			...
			if(currentObjMarkUnPaid.expenseCategory != 'Bill') {	
				if(confirm("This will delete all of instances of the expense in the system. Are you sure you want to delete this expense?")){
				...
				if(confirm("This will mark all instances of this bill in the system as UnPaid. Are you sure you want to completely mark this bill as UnPaid?")){	
